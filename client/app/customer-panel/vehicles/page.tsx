...
export default function CustomerVehiclesPage() {
  ...
  const { data: vehiclesResponse, isLoading } = useGetCustomerVehiclesQuery({
    page: currentPage,
    pageSize: pageSize,
  ...
  // Extract vehicles and total from the new response format
  const vehicles = vehiclesResponse?.items || [];
  const totalCount = vehiclesResponse?.totalCount || 0;
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <Button
          onClick={() => router.push("/customer-panel/vehicles/create")}
                ...
                {vehicles.map((vehicle: VehicleDto) => (
                  <TableRow key={vehicle.id}>
                    ...
                    <TableCell className="hidden lg:table-cell">
                      <Badge className={"text-xs"}>{vehicle.status}</Badge>
                    </TableCell>
